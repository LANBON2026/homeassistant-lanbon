...
class LanbonConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    VERSION = 1
    ...
    def __init__(self) -> None:
        ...
        # Authoritative: firmware uiLanguageGetConstStringLabel(pDevTypeSelTab[].refLanguEnum)
        self._type_name: str = "LANBON"

    def _set_type_name(self, name: Any | None) -> None:
        text = str(name).strip() if name is not None else ""
        self._type_name = text if text else "LANBON"
        # Discovery card primary line (secondary stays manifest "LANBON")
    ...
    async def async_step_zeroconf(self, discovery_info: ZeroconfServiceInfo) -> FlowResult:
        ...
        # zeroconf may deliver bytes keys/values
        norm = {
        ...
        try:
            self._sw_type = int(norm.get("sw_type")) if norm.get("sw_type") is not None else None
        except (TypeError, ValueError):
            self._sw_type = None
        # Prefer firmware type_name TXT; fall back to mDNS instance name
